...
public class RegexMapModule {
	...
	public static HashMap<Direction, Vector<MapLocation>> incrementalOffsets = new HashMap();
	...
	public static void archonExplore(Direction dir) throws Exception {
		...
		Vector<MapLocation> offsets = incrementalOffsets.get(dir);
		if (offsets == null) {
			offsets = new Vector();
			int r = RobotType.ARCHON.sensorRadius();
			boolean omni = dir == Direction.OMNI;
			for (int x = -r; x <= r; x++) {
				for (int y = -r; y <= r; y++) {
					MapLocation square = new MapLocation(locX + x, locY + y);
					if (omni) {
						if (rc.canSenseSquare(square)) {
							offsets.add(new MapLocation(x, y));
						}
					} else {
						MapLocation next = square.add(dir);
						if (rc.canSenseSquare(square)
								&& !rc.canSenseSquare(next)) {
							offsets.add(new MapLocation(x, y));
						}
					}
				}
			...
			if (!omni) {
				incrementalOffsets.put(dir, offsets);
			}
		}
