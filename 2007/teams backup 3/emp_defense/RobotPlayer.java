...
//    	}
    ...
    public static void fighterSpill() throws Exception {
    	MapLocation loc = rc.getLocation();
    	...
    	for (Robot r : rc.senseNearbyAirRobots()) {
    		if (r.getTeam() == team) {
	    		...
	    		if (loc.isAdjacentTo(loc2)) {
	    			double capacity = (energy - rc.senseEventualEnergonOf(r)) / 2;
	    			if (capacity > 0) {
		    			desired += capacity;
		    			neighborLocs[nextIndex] = loc2;
		    			neighborCapacitys[nextIndex] = capacity;
		...
		for (int i = 0; i < nextIndex; i++) {
			rc.transferEnergon(neighborCapacitys[i] / desired, neighborLocs[i], IN_AIR);
		}
    }
    ...
    public static void fighterHeal_helper(MapLocation pos) throws Exception {
    	Robot r = rc.senseAirRobotAtLocation(pos);
    	if (r != null && r.getTeam() == team) {
    		double energy = rc.getEnergonLevel();
    		if (energy > 2) {
    ...
    public static void fighterHeal(MapLocation base) throws Exception {
    	MapLocation loc = rc.getLocation();
...
//	            		}
	            	...
	            	if (type == ARCHON) {
		            	...
		            	rc.setIndicatorString(1, "production = " + rc.senseEnergonProductionOf(r));
...
//			            		explore(false);
	            	...
	            	if (type == SCOUT) {
//		            	rc.setIndicatorString(0, "action# = " + rc.getActionQueueSize());
//		            	rc.setIndicatorString(2, "dir = " + rc.senseEnemyArchon() + ", round: " + round());
	            		...
	            		if (rc.isActive()) {
		            		...
		            		if (base.distanceSquaredTo(towardEnemy) > 25) {
		            			towardEnemy = null;
		            		}
	        				...
	        				for (Robot r : rc.senseNearbyAirRobots()) {
	        					if (r.getTeam() != team) {
	        						MapLocation pos = rc.senseLocationOf(r);
	        						if (base.distanceSquaredTo(pos) <= 25) {
		        						int dist = loc.distanceSquaredTo(pos);
		        						RobotType t = rc.senseRobotTypeOf(r);
		        						if (t == SCOUT) {
		        							if (dist < bestScoutDist) {
		        								bestScoutDist = dist;
		        								bestScout = pos;
		        							}
		        						} else {
...
//		            		fighterHeal(toEnemy, distFromBase <= 16 ? 1 : 0);
//		            		
//	            			if (rc.getEnergonLevel() > 5) {
//			            		MapLocation towardEnemy = loc.add(toEnemy);
//			            		int dist2 = towardEnemy.distanceSquaredTo(base);
//			            		boolean moved = false;
//			            		if (dist2 <= 16) {
//			            			moved = tryMove(toEnemy);
//			            		} else if (dist2 <= 25 && rc.canMove(toEnemy)) {
//			            			moved = move(toEnemy);
//			            		} else if (distFromBase > 25) {
//			            			moved = tryMove(dir(base));
//			            		}
//			            		
//			            		if (!moved) {
//			            			scoutTryAttack();
