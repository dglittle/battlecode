...
//    	}
    ...
    public static void sendInfo(boolean everything) throws Exception {
    	...
    	m.ints = new int[] {hashRound(Clock.getRoundNum()), 72, newWalls, newUpgrades, newNogrades,
    			mapLeft, mapRight, mapTop, mapBottom, childID, id, loc.getX(), loc.getY()};
    	rc.queueBroadcast(m);
    }
        	...
        	if (m.ints != null && m.ints.length >= 2) {
        		if (m.ints[0] == roundHash) {
        			...
        			if (m.ints[1] == 72) {
        		    	...
        		    	if (id == m.ints[9]) {
        		    		motherID = m.ints[10];
        		    		motherLoc = new MapLocation(m.ints[11], m.ints[12]);
        		    	}
    ...
    public void run() {
    	try {
			...
			int lastPing = Clock.getRoundNum();
	        ...
	        while (true) {
	            try {
	            	...
	            	if (type == TANK) {
	            		...
	            		rc.setIndicatorString(2, "mother id = " + motherID);
	            		
	            		processMessages();
	            		updateMap();
	            		clearNewSets();
	            		
	            		if (lastGroundEnemiesUpdate < round() - 10) {
	            			groundEnemies = new MapLocation[0];
	            		}
	            		...
	            		if (rc.isActive()) {
	            			...
	            			if (groundEnemies.length > 0) {
	            				if (!tryKillBestGroundEnemy()) {
	            					rc.yield();
	            				}
	            			} else {
		            			if (tryMove(dirToMother(motherID))) {
	            	...
	            	if (type == ARCHON) {
		            	...
		            	if (lastHeardFromChild < round() - 100) {
		            		childID = Integer.MIN_VALUE;
		            	}
			            	...
			            	if (rc.getEnergonLevel() >= rc.getMaxEnergonLevel() - 1) {
			            		if (childID == Integer.MIN_VALUE) {
			            			if (trySpawnRobot(SOLDIER)) {
				            			...
				            			Robot r = rc.senseGroundRobotAtLocation(forward());
				            			childID = r.getID();
