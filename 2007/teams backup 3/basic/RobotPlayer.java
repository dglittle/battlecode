...
public class RobotPlayer implements Runnable {
    ...
    public static void trySpawnRobot(RobotType t) throws Exception {
    ...
    public static void tryArchonHealEveryone(double keepThisMuch, double mustBeThisDamaged) throws Exception {
    	try {
	    	...
	    	double fraction = (energy - keepThisMuch) / desired;  
...
//    			debug_printMapPath(goalPath);
    ...
    public void run() {
    	try {
    		int nextSendRound = Clock.getRoundNum();
    		final int sendInterval = 10;
    		
    		MapLocation exploreDest = null;
    		MapLocation lastLoc = rc.getLocation();
    		boolean sentWalls = false;
    		MapLocation firstUpgrade = null;
	        ...
	        while (true) {
	            try {
	            	if (type == ARCHON) {
	            		...
	            		if (round > nextSendRound) {
	            			sendWalls();
	            			nextSendRound = Clock.getRoundNum() + sendInterval;
	            		}
	            		
	            		tryArchonHealEveryone(40, 5);
	            		...
	            		if (rc.isActive()) {            			
	            			...
	            			if (rc.getEnergonLevel() > 59) {
	            				if (rand.nextBoolean()) {
	            					...
	            					trySpawnRobot(SOLDIER);
	            				}
	            			} else {
	            				tryMove(goalDir != null ? goalDir : dirToMyNearestArchon());
	            				if (loc.equals(archonGoal)) {
	            					archonGoal = null;
	            				}
	            				exploreForOthers();
	            				...
	            				for (Upgrade u : rc.senseNearbyUpgrades()) {
	            					MapLocation pos = sub(rc.senseLocationOf(u), mapOrigin);
	            					if (distMap[pos.getX()][pos.getY()] != null) {
	            						goalDir = distMap[pos.getX()][pos.getY()].firstDir;
	            					}
	            				}
	            				
	            				if (goalDir == null && archonGoal != null) {            				
		            				for (MapLocation onPath : goalPath) {
		            					if (distMap[onPath.getX()][onPath.getY()] != null) {
		            						goalDir = distMap[onPath.getX()][onPath.getY()].firstDir;
		            					}
		            				}
	            				}
	            			}
...
//            			}
	            	...
	            	if (type == SOLDIER) {
	            		double energy = rc.getEnergonLevel();
	            		if (energy > type.maxEnergon() - 1) {
	            			exploreDest = new MapLocation(0, 0);
	            		} else if (energy < 3) {
	            			exploreDest = null;
	            		}
            			...
            			if (!rc.isActive()) {
		            		if (exploreDest != null) {
		            			...
		            			int distSq = loc.distanceSquaredTo(base);
		            			if (distSq < 4) {
		            				tryMove(dir(base).opposite());
		            			} else if (distSq > 9) {
		            				tryMove(dir(base));
		            			}
		            		} else {
	            	...
	            	if (type == SCOUT) {
	            		double energy = rc.getEnergonLevel();
	            		if (energy > type.maxEnergon() - 1) {
	            			// let's go exploring... but we need to decide on a place to go
	            			exploreDest = new MapLocation(
        							rand.nextInt(mapXSize) + mapOriginX,
        							rand.nextInt(mapYSize) + mapOriginY);
	            		} else if (energy < type.maxEnergon() / 2) {
	            			if (exploreDest != null) {	            			
	            				sentWalls = false;
	            			}
	            			exploreDest = null;
            			...
            			if (!rc.isActive()) {
		            		if (exploreDest != null) {
		            			if (!tryMove(dir(exploreDest))) {
		            				exploreDest = null;
		            				sentWalls = false;
		            			} else {
	    	            			exploreForOthers();
	    	            			if (firstUpgrade == null) {
	    	            				Upgrade[] u = rc.senseNearbyUpgrades();
	    	            				if (u.length > 0) {
	    	            					firstUpgrade = rc.senseLocationOf(u[0]);
	    	            				}
	    	            			}
		            			}
		            		} else {
		            			MapLocation base = nearestArchonLocation();
		            			if (!sentWalls && loc.distanceSquaredTo(base) <= broadcastRadiusSquared) {
		            				sendWalls();
		            				sentWalls = true;
		            				if (firstUpgrade != null) {
		            					sendMessage("upgrade", new MapLocation[] {firstUpgrade});
		            					firstUpgrade = null;
		            				}
		            			}
		            			tryMove(dir(base));
