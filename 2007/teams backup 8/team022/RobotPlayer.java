    	...
    	if (badEmps != null) {
            ...
            for (Collection<MapLocation> bads : badCols) {
                ...
                for (MapLocation bad : bads) {
                    ...
                    if (!rc.canSenseSquare(bad) && distBase < closestDist) {
                        closestDist = distBase;
                        closest = bad;
                    }
                }
                if (closest != null) {
                    if (closestDist <= type.attackRadiusMaxSquared()) {
                		// blindly attack it
...
//                                        weight = -10000000;
	            	...
	            	if (type == SCOUT) {
	            		...
	            		if (sendMessage) {
	            			...
	            			for (Robot r : rc.senseNearbyAirRobots()) {
	            				if (r.getTeam() == team) {
	            					if (rc.senseRobotTypeOf(r) == SCOUT &&
	            							(rc.senseLocationOf(r).distanceSquaredTo(reqSamLoc) < loc.distanceSquaredTo(reqSamLoc) ||
	            							r.getID() < id)) {
	        	            			ind2 += " clos " + r;
	            						doSamResp = false;
	            						break scan;
