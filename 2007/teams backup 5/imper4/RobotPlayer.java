	        ...
	        while (true) {
	            try {
	            	if (type == ARCHON) {
	            		...
	            		tryArchonHealEveryone(5);
	            		
	            		if (rc.isActive()) {            			
	            		} else {
	            			if (rc.getEnergonLevel() > 25) {
...
//		            			}
	            	...
	            	if (type == SCOUT) {
//                        rc.setIndicatorString(2, "type scout");
	            		if (waiting) {
	            			if (rc.getEnergonLevel() > type.maxEnergon() - 1) {
	            				waiting = false;
                                rc.setIndicatorString(0, "no longer waiting");
	            			}
	            		}
	            		if (!waiting) {
                            ...
                            rc.setIndicatorString(0, "goal: " + goalScout + " attacking: "  + attacking + " currentaction: " + rc.getCurrentAction());
...
//                                rc.setIndicatorString(0, "active ");
		            			...
		            			if (attacking || (loc.distanceSquaredTo(base) <= 64 && best != null)) {
		            				attacking = true;
		            				if (best != null) {
		            			...
		            			MapLocation enemy = nearestArchonLocation();// TODO huh?
		            			...
		            			if (goalScout != null) {
		            				if (loc.equals(goalScout)) {
		            					if (rc.getEnergonLevel() >= 14.0 && 
		            						...
		            						sendMessage("goalScout", new MapLocation[] {loc}); // TODO encrypt?
		            						for (Robot r : rc.senseNearbyAirRobots()) {
		            							if (r.getTeam() != team) {
		            								MapLocation pos = rc.senseLocationOf(r);
