...
//			            		}
	            	...
	            	if (type == EMP) {
	            		if (!rc.isActive()) {
	            			...
	            			// recall that we were not moving away from mothers, and that scouts weren't healing the EMP
	            			// TODO add code to *evade* own mothers if they're too close
	            			// don't get closer to any of own mothers
	            			HashSet<MapLocation> motherLocs = new HashSet<MapLocation>();
                            boolean clear = true;
	            			for (MapLocation motherLoc : rc.senseAlliedArchons()) {
		            			if (motherLoc.distanceSquaredTo(loc) <= 36) {
		            				motherLocs.add(motherLoc);
                                    if (motherLoc.distanceSquaredTo(loc) <= 25) {
                                        clear = false;
                                    }
		            			}
	            			}
	            	    	...
	            	    	if (dirEnemy == OMNI) {
		            			...
		            			Direction[] ds = new Direction[] {
		            					dirEnemy,
		            					dirEnemy.rotateRight(),
		            					dirEnemy.rotateLeft(),
		            					dirEnemy.rotateRight().rotateRight(),
		            					dirEnemy.rotateLeft().rotateLeft(),
		            					};
		            			// TODO better path finding here? weigh boxes? eg:
		            			// - don't step into box surrounded by enemies
		            			// - step closer to enemy archon if you can see it
	//	            			boolean moved = false;
                                String ind = Clock.getRoundNum() + " " + dirEnemy.toString();
		            			search:
		            			for (Direction d : ds) {
		            		    	if (rc.canMove(d)) {
                                        ind += " " + d;
                                        boolean clear2 = true;
		            		    		for (MapLocation motherLoc : motherLocs) {
                                            ind += " " + motherLoc.distanceSquaredTo(loc.add(d));
		            		    			if (motherLoc.distanceSquaredTo(loc.add(d)) <= 25) {
                                                clear2 = false;
                                            }
		            		    		}
                                        if (clear2) {
                                ...
                                rc.setIndicatorString(0, ind);
		            			// if we're really stuck, attack?
	//	            			if (!moved) {
	//	            				Robot target = null;
	//	            				search:
	//	            				for (Robot r : rc.senseNearbyAirRobots()) {
	...
	//	            						case SCOUT:
	//	            							if (rc.senseEnergonLevelOf(r) < type.attackPower())
	//	            								break search;
	//	            						case EMP:
	//	            							target = r;
	//	            							if (rc.senseEnergonLevelOf(r) < type.attackPower())
