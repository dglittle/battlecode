...
public class RobotPlayer implements Runnable {
    static final int MIN_SAFE_SCOUT_COUNT = 23;
...
//    static int reqSamId = -1;
    ...
    public static MapLocation spotAwayFrom(MapLocation pos) {
    	MapLocation loc = rc.getLocation();
    	Direction away = dir(pos, loc);
    	int awayI = away.ordinal();
...
//    	badScouts = badEmps = null;
    ...
    public static void scoutHeal(MapLocation base) throws Exception {
    	...
    	final double altruistLevelFromTop = 4;
    	for (Robot r : rc.senseNearbyGroundRobots()) {
    		if (r.getTeam() == team) {
    			MapLocation pos = rc.senseLocationOf(r);
    			if (isClose(loc, pos)) {
    	...
    	while (true) {
            ...
            if (rc.canSenseSquare(pos)) {
                ...
                if (r != null && r.getTeam() == team) {
                    ...
                    {
                        double canTake = rc.senseMaxEnergonOf(r) - rc.senseEventualEnergonOf(r);
                        double canGive = rc.getEnergonLevel() - keep;
                        if (canGive <= 0) {
                        	break scan;
                        }
                        if (canTake > 0) {
                        ...
                        break;
                    case SCOUT:
                        if (base.distanceSquaredTo(pos) >= dist && rc.getEnergonLevel() > keep) {
                            scouts.add(r);
                        }
                        break;
                    }
    ...
    public static void soldierHeal(MapLocation base) throws Exception {
    	MapLocation loc = rc.getLocation();
    	
    	// heal air units, if they are in need
    	final double altruistLevelFromTop = 5;
    	for (Robot r : rc.senseNearbyAirRobots()) {
    		if (r.getTeam() == team) {
    			MapLocation pos = rc.senseLocationOf(r);
    			if (isClose(loc, pos)) {
	    			...
	    			double eventual = rc.senseEventualEnergonOf(r);
	    			double capacity = rc.senseMaxEnergonOf(r) - eventual;
	    			if (capacity > altruistLevelFromTop && energy + altruistLevelFromTop >= rc.getMaxEnergonLevel()) {
	    				rc.transferEnergon(Math.min(altruistLevelFromTop, rc.getMaxEnergonLevel() - energy), pos, ON_GROUND);
	    			}
    			}
    		}
    	...
    	while (true) {
            ...
            if (rc.canSenseSquare(pos)) {
                Robot r = rc.senseGroundRobotAtLocation(pos);
                if (r != null && r.getTeam() == team) {
                    if (base.distanceSquaredTo(pos) >= dist) {
                        scouts.add(r);
                    }
                    break;
                }
        ...
        for (Robot r : scouts) {
			...
			double capacity = rc.senseMaxEnergonOf(r) - eventual;
			if (energy > eventual && capacity > 0) {
                rc.transferEnergon(Math.min((energy - eventual) / 2, capacity), rc.senseLocationOf(r), ON_GROUND);
            }
        }
    }
    ...
    public static MapLocation nearestGroundEnemy() throws Exception {
    	MapLocation loc = rc.getLocation();
    	int bestDist = Integer.MAX_VALUE;
    	MapLocation bestLoc = null;
    	for (Robot r : rc.senseNearbyGroundRobots()) {
    ...
    public static int distFromNearest(MapLocation pos, MapLocation[] locs) {
    	int bestDist = Integer.MAX_VALUE;
    ...
    public static int distFromNearest(MapLocation pos, Collection<MapLocation> locs) {
    	int bestDist = Integer.MAX_VALUE;
    	for (MapLocation loc : locs) {
    		int dist = loc.distanceSquaredTo(pos);
    		if (dist < bestDist) {
    	...
    	try {
    		
    		{
	    		...
	    		if (antiEmp) {
		    		...
		    		RobotPlayer_huns a = new RobotPlayer_huns(rc);
		    		a.run();
	    		}
    		}   		
    		...
    		if (type == ARCHON) {
    			...
    			Direction enemy = rc.senseEnemyArchon();
    			boolean doIt = true;
    			if (enemy.isDiagonal()) {
    				if (archonQuadrant.equals(enemy)) {
    					face(enemy);
    				} else if (archonQuadrant.equals(enemy.rotateLeft().rotateLeft())) {
    					face(enemy.rotateLeft());
    				} else if (archonQuadrant.equals(enemy.rotateRight().rotateRight())) {
    					face(enemy.rotateRight());
    				} else {
    					doIt = false;
    				}
    			} else {
    				if (archonQuadrant.equals(enemy.rotateLeft())) {
    					face(enemy);
    				} else if (archonQuadrant.equals(enemy.rotateLeft().rotateLeft().rotateLeft())) {
    					face(enemy.rotateLeft().rotateLeft());
    				} else if (archonQuadrant.equals(enemy.rotateLeft().rotateRight().rotateRight())) {
    					face(enemy.rotateRight().rotateRight());
    				} else {
    					doIt = false;
    				}
    			}
    			if (doIt) {
					...
					rc.queueMoveForward();
					rc.queueMoveForward();
					while (rc.isActive() || rc.getActionQueueSize() > 0 || rc.getEnergonLevel() < 20) {
						rc.yield();
					}
					rc.queueSpawn(SCOUT);
					rc.yield();
					int round = round();
					loc = rc.getLocation();
					MapLocation forward = forward();
					Robot r = rc.senseAirRobotAtLocation(forward);
					while (rc.senseEnergonLevelOf(r) < 14.5) {
						if (rc.getEnergonLevel() > 1) {
							rc.transferEnergon(1, forward, IN_AIR);
						}
					}
					rc.yield();
    			}
			...
			boolean realScout = (type == SCOUT && (round() < 40));
			if (realScout) {
				MapLocation pos = nearestMother();
				MapLocation loc = rc.getLocation();
				Direction d = dir(pos, loc);
				int[] ds = MyConst.dirDs[d.ordinal()];
				scoutGoal = new MapLocation(loc.getX() + ds[0] * 40, loc.getY() + ds[1] * 40);
			}
			...
			boolean escortEmp = false;
			
			int preferredParent = rand.nextInt(rc.getUnitCount(ARCHON));
...
//	            		}
	            		...
	            		if (rc.isActive()) {
	            			...
	            			final int leashLengthSq = 8;
	            			MapLocation loc = rc.getLocation();
	            			int dist = mom.distanceSquaredTo(loc);
	            			i0 = "";

	            			p0("mom " + sub(mom, loc));
	            			if (rc.getEnergonLevel() < rc.getMaxEnergonLevel() / 2 || dist > leashLengthSq) {
		            			Direction dirGm = dirToGrandMother();
	            				if (tryMoveNotTooHard_notOnUpgrade(dirGm)) { p0("desp mv not2hard"); continue; }
	            				if (tryAttack()) { p0("desp attack"); continue; }
	            				if (tryMove(dirGm)) { p0("desp mv harder"); continue; }
	            				p0("desp stuck");
	            			} else {
		            			MapLocation nearestEnemy = nearestGroundEnemy();
		            			if (nearestEnemy != null && loc.isAdjacentTo(nearestEnemy)) {
		            				if (tryAttackGround(nearestEnemy)) { p0("norm proxim attack"); continue; }
		            				p0("norm proxim stuck");
		            			} else {
			            			if (nearestEnemy != null) {
				            			Direction dirToEnemy = dir(nearestEnemy);
				            			MapLocation spaceTowardEnemy = loc.add(dirToEnemy);
				            			if (spaceTowardEnemy.distanceSquaredTo(mom) <= leashLengthSq) {
				            				if (tryMove(dirToEnemy)) { p0("norm remote noleash mv 2 enemy"); continue; }
				            				p0("norm remote noleash stuck");
				            			} else {
				            				p0("norm remote leash");
				            			}
			            			}
			            			if (rand.nextBoolean()) {
		            					flip();
			            			...
			            			if (loc.add(rc.senseEnemyArchon()).distanceSquaredTo(mom) <= leashLengthSq) {
			            				if (tryMoveNotTooHard_notOnUpgrade(rc.senseEnemyArchon())) { p0("norm noleash mv 2 earch"); continue; }
			            				p0("norm noleash stuck " + dirstrs[rc.senseEnemyArchon().ordinal()]);
			            			} else {
			            				p0("norm leash");
			            			}
		            			}
...
//		            	}
	            		...
	            		if (rc.isActive()) {
		            			...
		            			if (r.getTeam() != team) {
		            				...
		            				if (t == EMP) {
		            					if (rc.senseRoundsUntilIdle(r) < 20) {
		            						...
		            						approachTarget = spotAwayFrom(pos);
			            	...
			            	if (rc.getEnergonLevel() >= 45) {
			            		...
			            		if (rand.nextBoolean()) {			            		
				            		if (rc.getUnitCount(SCOUT) < 25) {
				            			if (trySpawnRobot(SCOUT)) continue;
				            		}
				            		if (rc.getUnitCount(SOLDIER) + rc.getUnitCount(TANK) < 8) {
...
//			            		}
			            	...
			            	if (round() > eggReady) {
			            		...
			            		if (leader) {
			            			while (true) {
				            			if (approachTarget != null && Clock.getRoundNum() - approachTargetTime < 50) {
			            		...
			            		if (leader) {
				            		for (Robot r : rc.senseNearbyGroundRobots()) {
				            			if (r.getTeam() != team) {
				            				...
				            				if (t == SOLDIER) {
					            				MapLocation pos = rc.senseLocationOf(r);
					            				if (loc.distanceSquaredTo(pos) <= 4) {
					            					d = dir(pos, loc);
					            				}
				            				}
				            			}
...
//			            		if (leader && !tryingToRunAway) {
			            		...
			            		if (leader || (leaderIndex % 2 == 0)) {
				            		...
				            		if (scoutCount >= 4 && !tryingToRunAway) {
				            			attackMode = true;
...
//		        				}
	            		...
	            		if (!rc.isActive()) {
	            			...
	            			if (shouldEvolveEmp && rc.getEnergonLevel() > EMP.spawnCost() + 2) {
	            				// TODO duplicate enemy weighing scan, just to be sure?
		            			for (MapLocation mother : rc.senseAlliedArchons()) {
		            				if (mother.distanceSquaredTo(loc) <= 9) {
...
//		            				}
	        				...
	        				int startBcc = Clock.getBytecodeNum();
	        				boolean optimEmp = false; /*rc.getUnitCount(SCOUT) >= MIN_SAFE_SCOUT_COUNT &&
	        					rc.getEnergonLevel() >= 14;*/
	        				rc.setIndicatorString(0, "#S " + rc.getUnitCount(SCOUT));
...
//		        						}
		        				...
		        				if (ourWeight < TOOMUCH && archonLoc != null || optimEmp) {
		        					ourWeight = 0;
		        					Robot candidate = null;
		        					double candidateE = MIN_EMP_ENERGON;
//		        					Robot furthestR = null;
//		        					int furthestDist = Integer.MIN_VALUE;
		        					Robot furthestR = rc.getRobot();
		        					int[] ds = MyConst.dirDs[rc.senseEnemyArchon().ordinal()];
		        					MapLocation fakeEnemy = new MapLocation(loc.getX() + ds[0] * 40, loc.getY() + ds[1] * 40);
		        					int furthestDist = leaderPos().distanceSquaredTo(loc);
		        					int closestToEnemy = loc.distanceSquaredTo(fakeEnemy);
			        				...
			        				for (Robot r : rc.senseNearbyAirRobots()) {
			        					if (r.getTeam() != team) {
											...
											case SCOUT:
												...
												if (optimEmp) {
													MapLocation pos = rc.senseLocationOf(r);
													int distFromEnemy = pos.distanceSquaredTo(fakeEnemy);
													
													if (rc.senseEnergonLevelOf(r) >= 14) {
														if (distFromEnemy < closestToEnemy || (distFromEnemy == closestToEnemy && 
																r.getID() < furthestR.getID())) {
															closestToEnemy = distFromEnemy;
															furthestR = r;
														}
													}
//													switch (rc.senseEnemyArchon()) {
//													case NORTH: 
//													}
//													int dist = leaderPos().distanceSquaredTo(rc.senseLocationOf(r));
//													if (rc.senseEnergonLevelOf(r) > 14 && dist > furthestDist) {
//														furthestR = r;
//														furthestDist = dist;
//													}
												}
												if (e > candidateE) {
													...
													for (MapLocation mother : rc.senseAlliedArchons()) {
														if (rloc.distanceSquaredTo(mother) <= 9 || optimEmp) {
															nearMother = true;
															break;
														}
													}
			        				...
			        				if (ourWeight >= 5) {
			        					if (candidate != null && ourWeight >= 2 * enemyWeight && archonLoc != null) {
				        					...
				        					roundHeardEmpMode = Clock.getRoundNum();
			        					} else if (optimEmp && enemyWeight == 0 && furthestR == rc.getRobot()) {
			        						shouldEvolveEmp = true;
			        					}
			        				}
			        				rc.setIndicatorString(2,
			        						...
			        						(shouldEvolveEmp ? " slf! " : "") +
//			        						" OE " + optimEmp +
//			        						" furthR==self " + (furthestR == rc.getRobot()) +
			        						(candidate == null ? "" : " cand " + candidate.getID() + "!") +
			        						" enemy " + enemyWeight +
			        						" us " + ourWeight +
			        						" found " + (archonLoc != null) +
			        						" furID " + furthestR.getID() +
			        						" candE " + candidateE +
                            ...
                            MapLocation bestScout = null;
                            MapLocation bestEmp = null;
                            MapLocation friendEmp = null;
//                            int friendDist = Integer.MIN_VALUE;
//                            int myDist = leaderPos().distanceSquaredTo(loc);
                            MapLocation step = loc.add(rc.senseEnemyArchon());
                            MapLocation target = new MapLocation(loc.getX() + 1000 * (step.getX() - loc.getX()),
                            		loc.getY() + 1000 * (step.getY() - loc.getY()));
                            int friendDist = Integer.MAX_VALUE;
                            int myDist = loc.distanceSquaredTo(target);
							double bestHealth = Double.POSITIVE_INFINITY;
                            Vector<MapLocation> vScouts = new Vector<MapLocation>();
                            Vector<MapLocation> vEmps = new Vector<MapLocation>();
                            for (Robot r : rc.senseNearbyAirRobots()) {
...
//                                		int dist = leaderPos().distanceSquaredTo(pos);
//                                		if (dist > myDist && dist > friendDist) {
//                                			friendDist = dist;
//                                			friendEmp = rc.senseLocationOf(r);
//                                		}
                                		int dist = rc.senseLocationOf(r).distanceSquaredTo(target);
                                		if (dist < myDist && dist < friendDist) {
                                			friendDist = dist;
                                			friendEmp = rc.senseLocationOf(r);
                                		}
...
//	        				}
	            			...
	            			if (!weSentEMPMessage && rand.nextInt(10) == 0) {
	            				...
	            				// and if there is just one or two guys, approach them
	            				{
			            			...
			            			double prodSum = Double.MAX_VALUE;
			            			
			            			double badness = 0;
			            			
			            			int closest = Integer.MAX_VALUE;
			            			MapLocation closestPos = null;
			            			
			            			for (Robot r : rc.senseNearbyGroundRobots()) {
			            				if (r.getTeam() != team) {
			            					RobotType t = rc.senseRobotTypeOf(r);
			            					MapLocation pos2 = rc.senseLocationOf(r);
			            					int dist = loc.distanceSquaredTo(pos2);
			            					if (dist < closest) {
			            						closest = dist;
			            						closestPos = pos;
			            					}
			            					if (t == ARCHON) {
			            						...
			            						prodSum += rc.senseEnergonProductionOf(r);
			            					}
			            					if (t == TANK) {
			            						tankCount++;
			            					}
			            			...
			            			badness += tankCount * 5;
			            			badness += soldierCount * 2;
			            			double goodness = 0.9 * rc.getUnitCount(SOLDIER);
			            			...
			            			} else if (closestPos != null && (badness > goodness || goodness < 3)) {
			            				sendAvoidMessage(closestPos);
			            			} else if (pos != null && (prodSum <= 0.5 && goodness > badness * 1.5)) {
	        				...
	        				if (rc.getEnergonLevel() > 5) {	        				
		        				while (true) {
			        				...
			        				if ((escortEmp || bestEmp == bestScout) && friendEmp != null &&
			        				    	rc.getEnergonLevel() > keepAmt()) {
			        					if (tryMove(dir(friendEmp))) {
			        						escortEmp = true;
			        						break;
			        					}
			        				} else {
	...
	static void p0(String str) {
		i0 += str + "; ";
		rc.setIndicatorString(0, Clock.getRoundNum() + " " + i0);
	}

	static double keepAmt() {
		return 2 + .5 * Math.max(0, rc.getLocation().distanceSquaredTo(nearestMother()) - 5);
	}
