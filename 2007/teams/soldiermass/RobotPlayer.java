...
//    static int reqSamId = -1;
    ...
    public static void doHealing() throws Exception {
    	...
    	} else if (type == TANK || type == SAM || type == SOLDIER) {
    		{
	    		for (Robot r : rc.senseNearbyGroundRobots()) {
	    			if (r.getTeam() == team) {
	    				MapLocation pos = rc.senseLocationOf(r);
...
//                        }
	            	...
	            	if (type == SOLDIER) {
	            		...
	            		if (rc.isActive()) {
	            			...
	            			Robot closest = null;
	            			MapLocation nearestMother = nearestMother();
	            			if (rc.getEventualEnergonLevel() < type.maxEnergon() / 2) {
	            				if (!loc.isAdjacentTo(nearestMother))
	            					tryMove(dir(nearestMother));
	            			} else {
		            			for (Robot r : rc.senseNearbyGroundRobots()) {
		            				if (r.getTeam() != team) {
		            					int dist = loc.distanceSquaredTo(rc.senseLocationOf(r));
		            					if (dist < closestDist || dist == closestDist &&
		            							rc.senseEnergonLevelOf(r) < rc.senseEnergonLevelOf(closest)) {
		            						closestDist = dist;
		            						closest = r;
		            					}
		            				}
		            			...
		            			if (closest != null) {
			            			MapLocation closestLoc = rc.senseLocationOf(closest);
			            			if (loc.isAdjacentTo(closestLoc)) {
			            				if (!rc.canAttackSquare(closestLoc)) {
			            					rc.queueSetDirection(dir(closestLoc));
			            					rc.yield();
			            				}
			            				rc.queueAttackGround(closestLoc);
			            			} else {
		        						if (tryMoveNotTooHard(dirToSpot(closestLoc, ""))) {
		        							tryMove(rc.senseEnemyArchon());
		        						}
			            			}
		            			} else if (rc.getUnitCount(SOLDIER) > 10 && loc.distanceSquaredTo(nearestMother) <= 3) {
		            				tryMove(rc.senseEnemyArchon());
		            			}
	            			}
	            		}
