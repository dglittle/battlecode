...
public class IndependentMineTopLevel {
  ...
  public static final void _equipSensor() {
    while (true) {
      ...
      try {
        X.build(mineSensor, S.location, S.level);
        break;
      }
      catch(GameActionException e) {
        e.printStackTrace();
  ...
  public static final boolean _enemiesSpottedNearby() {
    for (RobotInfo robotInfo : S.nearbyRobotInfo()) {
      if (robotInfo == null) {
        continue;
      }
  ...
  public static RobotInfo[] _adjacentDefenseInfo = new RobotInfo[MapUtil.ADJACENT_DIRECTIONS];
    ...
    try {
      if (!hasSensor && capacityLeft >= _defenseSensorWeight) {
        if (S.flux > _defenseSensorCost && S.dFlux > 0) {
          X.build(defenseSensor, info.location, info.robot.getRobotLevel());
          return true;
        }
      }
      else if (capacityLeft >= _defenseWeaponWeight && S.flux > _defenseWeaponCost && S.dFlux > 0) {
        X.build(defenseWeapon, info.location, info.robot.getRobotLevel());
        return true;
      }
      return false;
    }
