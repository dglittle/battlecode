...
public class M {
   ...
   */
  public static final boolean scannedDirection[][][] =
      new boolean[arrayWidth][arrayHeight][MapUtil.ADJACENT_DIRECTIONS];
  ...
  /** dx for all map locations on the edge of the sensor range for the robot's orientation. */
  public static int sensorEdgeDx[];
  /** dy for all map locations on the edge of the sensor range for the robot's orientation. */
  ...
  /**
   * Map initialization used if the robot doesn't receive bearings from an archon for some time.
   */
   ...
   * 
   * This method should not be called directly. It is called by the *Init() methods.
   * 
  ...
  public static final void updateMapFromSensors() {
    ...
    if (S.sensorIsOmnidirectional) {
      if (!scanned[ax][ay]) {
        if (M.lastUpdateLocation != null && M.lastUpdateLocation.isAdjacentTo(S.location) &&
            M.lastUpdateDirectionInt == S.directionInt) {
   ...
   *
   * This method should not be called directly. It is called by updateMapAfter* methods.
   */
  public static final void updateMapFromSensorVector(int[] dxVector, int[] dyVector) {
    for (int i = 0; i < dxVector.length; i++) {
      int dx = dxVector[i], dy = dyVector[i];
      int mx = S.locationX + dx, my = S.locationY + dy;
      
      if (mx < mapMinX || mx > mapMaxX || my < mapMinY || my > mapMaxY) continue;
      
      int ax = mx - arrayBaseX, ay = my - arrayBaseY;
      if (known[ax][ay]) continue;
        
      TerrainTile tile = S.rc.senseTerrainTile(new MapLocation(mx, my));
      if (tile == null) {
        // Fake an exception so we'll fail tests.
        try {
          throw new GameActionException(GameActionExceptionType.CANT_SENSE_THAT,
  ...
  public static boolean landAtDirection(int i) {
    ...
    int ax = S.locationX + dx - M.arrayBaseX, ay = S.locationY + dy - M.arrayBaseY;
    return land[ax][ay];
  }
