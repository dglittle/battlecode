...
import heavy1.core.xconst.SensorRanges;
...
public class M {
   ...
   */
  public static boolean scannedDirection[][][];
  
  /** The sensor used to produce the map cache. */
  public static ComponentType scannedSensorType;
  ...
  /**
   * dx for all map locations on the back edge of the sensor range for the
   * robot's orientation.
   */
  public static int sensorBackEdgeDx[];
  /**
   * dy for all map locations on the back edge of the sensor range for the
   * robot's orientation.
   */
  public static int sensorBackEdgeDy[];
  
  /** The {@link SensorController} used for the map. */
  public static SensorController mapSensor;
  
  /** If true, the mapping code will be skipped. */
  public static boolean mapDisabled;
  
  /**
   * Updates map data after the robot gains a sensor.
   */
  public static final void updateMapAfterNewSensor() {
    if (mapDisabled) { return; }
    
    M.mapSensor = S.sensorController;
    SensorRanges.initSensorDxDy();
    if (S.sensorType != M.scannedSensorType) {
      // Sensor upgrade, we need to re-scan old locations.
      if (S.sensorIsOmnidirectional) {
        M.scanned = new boolean[arrayWidth][arrayHeight];
        M.scannedDirection = null;
      }
      else {
        M.scannedDirection = 
            new boolean[arrayWidth][arrayHeight][XDirection.ADJACENT_DIRECTIONS];
        M.scanned = null;
      }      
    }
  ...
  public static final void updateMapAfterTurning() {
    ...
    M.sensorBackEdgeDx = S.sensorBackEdgeDxByDirection[S.directionInt];
    M.sensorBackEdgeDy = S.sensorBackEdgeDyByDirection[S.directionInt];
  ...
  /** Disables map updating. */
  public static final void disableMapUpdates() {
    M.mapDisabled = true;
    if (M.mapSensor != null) {
      M.mapSensor = null;
      M.sensorDx = M.sensorDy = M.sensorEdgeDx = M.sensorEdgeDy =
          M.sensorBackEdgeDx = M.sensorBackEdgeDy = null;
    }
  }
  
  /** Re-enables map updating, after it has been disabled.*/
  public static final void enableMapUpdates() {
    if (M.mapDisabled) {
      M.mapSensor = S.sensorController;
  ...
  public static final void updateMapFromSensors() {
    // $ +stubs
    final int ax = S.locationX - M.arrayBaseX;
    final int ay = S.locationY - M.arrayBaseY;
    if (S.sensorIsOmnidirectional) {
      if (!scanned[ax][ay]) {
        final int backAX = ax - XDirection.intToDeltaX[S.directionInt];
        final int backAY = ay - XDirection.intToDeltaY[S.directionInt];
        
        if (scanned[backAX][backAY]) {
          updateMapFromSensorVector(M.sensorEdgeDx, M.sensorEdgeDy);
        } else {
          final int frontAX = ax + XDirection.intToDeltaX[S.directionInt];
          final int frontAY = ay + XDirection.intToDeltaY[S.directionInt];
          if (scanned[frontAX][frontAY]) {
            updateMapFromSensorVector(M.sensorBackEdgeDx, M.sensorBackEdgeDy);
          } else {
      ...
      if (!scannedDirection[ax][ay][S.directionInt]) {
        final int backAX = ax - XDirection.intToDeltaX[S.directionInt];
        final int backAY = ay - XDirection.intToDeltaY[S.directionInt];
        if (scannedDirection[backAX][backAY][S.directionInt]) {
          updateMapFromSensorVector(M.sensorEdgeDx, M.sensorEdgeDy);
        } else {
          final int frontAX = ax + XDirection.intToDeltaX[S.directionInt];
          final int frontAY = ay + XDirection.intToDeltaY[S.directionInt];
          if (scannedDirection[frontAX][frontAY][S.directionInt]) {
            updateMapFromSensorVector(M.sensorBackEdgeDx, M.sensorBackEdgeDy);
          } else {
            updateMapFromSensorVector(M.sensorDx, M.sensorDy);
          }
   ...
   * methods.
   */
  public static final void updateMapFromSensorVector(final int[] dxVector,
      final int[] dyVector) {
    for (int i = dxVector.length - 1; i >= 0; i--) {
      final int dx = dxVector[i], dy = dyVector[i];
      final int mx = S.locationX + dx, my = S.locationY + dy;

      if (mx < mapMinX || mx > mapMaxX || my < mapMinY || my > mapMaxY)
        continue;

      final int ax = mx - arrayBaseX, ay = my - arrayBaseY;
      if (known[ax][ay])
        continue;

      TerrainTile tile = S.rc.senseTerrainTile(new MapLocation(mx, my));
      if (tile == null) {
        D.debug_logException("Can't sense a tile in sensor range");
        continue;
      }
    ...
    for (int i = nearbyRobotInfos.length - 1; i >= 0; i--) {
      ...
      switch (info.chassis.ordinal()) {
        case XChassis.BUILDING_INT: {
          ...
          final int mx = location.x - M.arrayBaseX;
          final int my = location.y - M.arrayBaseY;
  ...
  /**
   * Updates {@link M#passable} to reflect a new obstacle.
   * 
   * This is used when building units.
   * 
   * @param location the location of the new obstacle
   */
  public static final void registerObstacle(MapLocation location) {
    final int mx = location.x - M.arrayBaseX;
    final int my = location.y - M.arrayBaseY;
