...
import heavy1.core.xconst.XComponentClass;
...
public final class S {
   ...
   */
  public static final boolean jumpControllerAvailable() {
    if (S.jumpControllers == null) return false;
    for (JumpController jc : S.jumpControllers) {
      if (!jc.isActive()) return true;
    }
    return false;
  }
  ...
  public static final void _addWeapon(WeaponController newWeapon) {
    ...
    WeaponController[] newWeaponControllers =
        new WeaponController[newWeaponCount];
  ...
  public static final void _addJump(JumpController newJump) {
    final int oldJumpCount =
        (S.jumpControllers == null) ? 0 : S.jumpControllers.length;
  ...
  public static final void _senseRobots() {
    ...
    try {
      ...
      // TODO(pwnall): consider sticking self's RobotInfo in here
    } catch (NullPointerException e) {
      // Filed as bug on forum.
      D.debug_logException(e);      
    }
  }
  
  /** Complains if any robot in nearbyRobots is null. */
  private static final void debug_checkNearbyRobots() {
    for (int i = _nearbyRobots.length - 1; i >= 0; i--) {
      if (_nearbyRobots[i] == null) {
        D.debug_pv("nearbyRobots has null entry!");
        D.debug_pv(Arrays.deepToString(_nearbyRobots));
        return;
      }
    }
  }
