...
public class TrainedHeavySoldier {
    ...
    while (true) {
      ...
      if (!S.movementController.isActive()) {
        ...
        if (bestTarget != null) {
          ...
          // Training -->
          int distSqToTarget = S.location.distanceSquaredTo(bestTarget);
          ...
          // If we're not scheduled to turn, then we need to analyze the distances.
            ...
            if (nextDirection.ordinal() >= 8) {
              ...
              if (nextLocation.distanceSquaredTo(bestTarget) <= ComponentType.BLASTER.range && S.movementController.canMove(directionAwayFromTarget)) {
