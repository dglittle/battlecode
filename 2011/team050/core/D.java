...
import team050.core.xconst.PrintingTags;
...
public class D {
  ...
  /**
   * Wraps {@link System.out.println(String s)} but only runs in debug mode.
  ...
  public static final void debug_pl(String tag, String s) {
    if (PrintingTags.YAFIM) {
      System.out.println("*** " + tag + ":" + Clock.getBytecodeNum() + ":" + S.location + ": " + s);
    }
  }
  
  public static final String debug_offset(MapLocation location) {
    return S.location.distanceSquaredTo(location) + " " + S.location.directionTo(location);
  }
  
  public static final String debug_offset(MapLocation from, MapLocation to) {
    return from.distanceSquaredTo(to) + " " + from.directionTo(to);
  }
