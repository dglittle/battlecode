...
import legenClone.util.Utilities;
...
public class BugPathfinding {
  ...
  public static final Direction bugTo(MapLocation goal) {
    return bugTo(goal, 0);
  }

  public static final Direction bugTo(MapLocation goal, int withinDistSq) {
    MapLocation here = S.location;

    // see if we made it
    int dist = here.distanceSquaredTo(goal);
    if (dist <= withinDistSq) {
      return Direction.NONE;
    }

    // update our goal,
    // and reset our state if we have a new goal
    if (BugPathfinding.goal == null || !BugPathfinding.goal.equals(goal)) {
      bugging = 0;
      BugPathfinding.goal = goal;
    }
    ...
    for (int i = 0; i < 8; i++) {
      if (S.movementController.canMove(bugDir)) {
        ...
        if (S.sensorController != null
            && S.sensorController.canSenseSquare(checkOffMap)) {
    ...
    // hm.. we tried all directions, and we're stuck,
    // we could throw an exception,
    // or pretend that we arrived
    bugging = 0;
